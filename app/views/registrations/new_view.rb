  ...
  class NewView < ApplicationComponent
    ...
    def render_logo
      div(class: "text-center mb-8") do
        ...
        p(class: "text-lg text-base-content/70") { "Создайте аккаунт для начала работы" }
      end
    end

    def render_registration_card
      Card :base_100, class: "shadow-2xl" do |card|
        card.body do
          card.title(class: "text-2xl mb-6 justify-center") { "Регистрация" }
    ...
    def render_errors
      Alert :error do
        ...
        div do
          h3(class: "font-bold") { "Ошибки при регистрации:" }
    ...
    def render_registration_form
      form(action: signup_path, method: "post", class: "space-y-4") do
        ...
        FormControl do
          label(class: "label") do
            span(class: "label-text") { "Подтверждение пароля" }
          end
          Input(
            type: "password",
