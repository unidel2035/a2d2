...
module MaintenanceMailer
  class UpcomingMaintenanceOperatorView < ApplicationComponent
    def initialize(maintenance:, operator:)
    ...
    def view_template
      div(class: "container") do
        div(class: "header") do
          h2 { "Уведомление о техническом обслуживании робота" }
        end

        div(class: "content") do
          p { "Здравствуйте, #{@operator.name}!" }

          render_notification_message

          div(class: "details") do
            h3 { "Информация о техническом обслуживании:" }
          ...
          p { "Пожалуйста, убедитесь, что робот полностью завершит текущую работу и будет выключен перед началом техническое обслуживания." }

          if @maintenance.description.present?
            div(class: "details") do
              h4 { "Примечания специалиста:" }
    ...
    def render_notification_message
      ...
      if days_before == 1
        p(class: "urgent") do
          text "⚠️ Внимание: завтра запланировано техническое обслуживание робота."
        end
      elsif days_before == 3
        p { "Напоминаем, что через 3 дня робот будет отправлен на техническое обслуживание." }
      else
        p { "Робот будет отправлен на техническое обслуживание через #{days_before} дней." }
      end
    end
