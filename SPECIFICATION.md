# Техническая спецификация проекта DronDoc 2025

## 1. Введение

### 1.1 Цель документа
Данный документ содержит техническую спецификацию системы управления документацией и полетными данными БПЛА "DronDoc 2025".

### 1.2 Область применения
Система предназначена для коммерческих организаций, эксплуатирующих беспилотные летательные аппараты для:
- Аэрофотосъемки
...
- **Полетное задание** - задача, выполняемая БПЛА в рамках одного полета
- **Телеметрия** - данные, получаемые от БПЛА в процессе полета
- **Оператор** - лицо, управляющее БПЛА

## 2. Общие требования

### 2.1 Функциональные требования

#### 2.1.1 Управление дронами
- Регистрация новых БПЛА в системе
- Хранение технических характеристик (вес, грузоподъемность, радиус действия)
- Учет серийных номеров и государственных регистрационных знаков
- Отслеживание статуса (в эксплуатации, на ТО, в ремонте, списан)
...
#### 2.1.3 Полетные операции
- Планирование полетов с указанием:
  - Даты и времени
  - Маршрута полета (геолокация)
  - Назначенного БПЛА
  - Назначенного оператора
  - Цели полета
- Автоматическая регистрация полета при загрузке данных телеметрии
- Хранение полетных логов
- Метеорологические данные на момент полета

#### 2.1.4 Техническое обслуживание
- Создание графика ТО на основе:
  - Налета часов
  - Календарных сроков
  - Количества циклов взлет-посадка
- Уведомления за 7 дней до планового ТО
- Регистрация выполненных работ
- Учет заменённых компонентов
- История ТО для каждого БПЛА
...
- Поддержка до 1000 БПЛА в системе
...
class Drone < ApplicationRecord
  ...
  - max_payload: decimal       # Грузоподъемность (кг)
  - max_range: decimal         # Радиус действия (км)
  - max_altitude: decimal      # Максимальная высота (м)
  - max_speed: decimal         # Максимальная скорость (км/ч)
  - battery_capacity: integer  # Емкость батареи (mAh)
  ...
  - total_flights: integer     # Общее количество полетов
...
class Flight < ApplicationRecord
  # Атрибуты
  - flight_number: string      # Номер полета
  ...
  - purpose: string            # Цель полета
  - location: string           # Местоположение
  - route: json                # Маршрут (GeoJSON)
  - weather_conditions: string # Метеоусловия
...
class MaintenanceRecord < ApplicationRecord
  ...
  - flight_hours_at_maintenance: decimal # Налет на момент ТО
...
#### 5.1.1 Эндпоинты дронов
```
GET    /api/v1/drones           # Список дронов
GET    /api/v1/drones/:id       # Информация о дроне
POST   /api/v1/drones           # Создание дрона
PATCH  /api/v1/drones/:id       # Обновление дрона
DELETE /api/v1/drones/:id       # Удаление дрона
```

#### 5.1.2 Эндпоинты полетов
```
GET    /api/v1/flights          # Список полетов
GET    /api/v1/flights/:id      # Информация о полете
POST   /api/v1/flights          # Создание полета
PATCH  /api/v1/flights/:id      # Обновление полета
POST   /api/v1/flights/:id/telemetry  # Загрузка телеметрии
```
...
#### Operator
- Создание и управление полетами
- Просмотр дронов
...
- Недели 3-4: Модуль управления дронами
- Недели 5-6: Модуль полетов
...
- Недели 17-18: Мобильное приложение
- Недели 19-20: Интеграции, финальное тестирование
